...
def odds_database_search(csv_file):
    ...
    start_time = time.time()
    dataframe['ISO3'] = dataframe.apply(lambda x: coco.convert(names=str(x['country']).replace("-", " "), to='ISO3'),
                                        axis=1)
...
def tournament_odds_scraper(url):
    if url_is_alive(url):
        print("URL IS ALIVE.")
        ...
        wait_pagination = WebDriverWait(driver, 100).until(EC.element_to_be_clickable((By.ID, 'pagination')))

        # Get the last page of the pagination.
        last_page_number = int(soup.find('div', id='pagination').find_all('a')[-1]['x-page'])
        ...
        for i in range(1, last_page_number + 1):
            ...
            if i is 1:
                ...
                soup = BeautifulSoup(driver.page_source, 'html.parser')
                tot_urls.append(matchUrls(soup))
        ...
        """
               ...
               tournament_tbl_soup = BeautifulSoup(tournament_tbl_html, "html.parser")
                 print(tournament_tbl_soup)      
                  odds_page = urlopen(url)
                  content = odds_page.read()  # get content from webpage
        page_soup = BeautifulSoup(content, 'lxml')

        tag = page_soup.find("span",
                             class_="visuallyhidden")  # string "class" is reserved for python itself,so bs use string "class_"
        print(tag)

        # print(page_soup)
        cont = page_soup.find('div', class_='hidden')
...
# urls = tournament_odds_scraper("http://www.oddsportal.com/tennis/united-kingdom/atp-wimbledon/results/")
# print(urls)
# odds_database_search("world_tennis_odds.csv")
